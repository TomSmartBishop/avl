...
namespace avl {
	...
	avl_ainl_res constexpr auto sub_mk(const v2& vec, const v2& other) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
		return rem_const_ref_t< decltype(vec) > { get<0>(vec) - get<0>(other), get<1>(vec) - get<1>(other) };
	}
	
	avl_ainl_res constexpr auto sub_mk(const v2& vec, const sc scalar) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
		return rem_const_ref_t< decltype(vec) > { get<0>(vec) - scalar, get<1>(vec) - scalar };
	}
	...
	avl_ainl_res constexpr auto mul_mk(const v2& vec, const v2& other) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
		return rem_const_ref_t< decltype(vec) > { get<0>(vec) * get<0>(other), get<1>(vec) * get<1>(other) };
	}
	
	avl_ainl_res constexpr auto mul_mk(const v2& vec, const sc scalar) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
		return rem_const_ref_t< decltype(vec) > { get<0>(vec) * scalar, get<1>(vec) * scalar };
	}
	...
	avl_ainl_res constexpr auto div_mk(const v2& vec, const v2& other) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
		assert(get<0>(other)!=decltype(other[0]){0});
		assert(get<1>(other)!=decltype(other[1]){0});
		return rem_const_ref_t< decltype(vec) > { get<0>(vec) / get<0>(other), get<1>(vec) / get<1>(other) };
	}
	
	avl_ainl_res constexpr auto div_mk(const v2& vec, const sc scalar) noexcept {//-> rem_const_ref_t<decltype(vec)> { // -> v2 {
	...
	avl_ainl_res constexpr auto len_sqr(const v2& vec) noexcept -> cmp_t<v2> {
		...
		return comp0 * comp0 + comp1 * comp1;
	}
	...
	/*
	avl_ainl_res constexpr auto dot(const v2& vec, const sc x, const sc y) noexcept -> cmp_t<v2> {
		return get<0>(vec) * x + get<1>(vec) * y;
	}
	*/
